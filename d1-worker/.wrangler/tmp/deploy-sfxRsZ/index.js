...
var DEFAULT_PAGES = {
  agency: { title: "\uC824\uB9AC\uACBD\uCC30\uCCAD \uAE30\uAD00 \uC18C\uAC1C", content: "\uC824\uB9AC \uACBD\uCC30\uCCAD\uC740 \uC2DC\uBBFC\uC758 \uC548\uC804\uACFC \uC9C8\uC11C\uB97C \uC704\uD574 \uC874\uC7AC\uD569\uB2C8\uB2E4." },
  rank: {
    title: "\uC824\uB9AC\uACBD\uCC30\uCCAD \uC9C1\uAE09\uD45C",
    high: { "\uCE58\uC548\uCD1D\uAC10": "", "\uCE58\uC548\uC815\uAC10": "", "\uCE58\uC548\uAC10": "" },
    mid: { "\uACBD\uBB34\uAD00": "", "\uCD1D\uACBD": "", "\uACBD\uC815": "", "\uACBD\uAC10": "" },
...
router.delete("/api/complaints/:id", async (req, env) => {
  if (!isAdmin(req, env)) return unauthorized();
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) return json({ error: "bad_id" }, { status: 400 });
...
router.delete("/api/suggestions/:id", async (req, env) => {
  if (!isAdmin(req, env)) return unauthorized();
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) return json({ error: "bad_id" }, { status: 400 });
