...
async function getOrSeedPage(env, key) {
  ...
  await env.DB.prepare("INSERT OR REPLACE INTO pages(page_key, page_json, updated) VALUES(?, ?, ?)").bind(key, JSON.stringify(data), (/* @__PURE__ */ new Date()).toISOString()).run();
  return data;
}
...
router.get("/api/complaints", async (req, env) => {
  if (!isAdmin(req, env)) return unauthorized();
  const { results } = await env.DB.prepare("SELECT id, name, identity, content, created, fileName, fileKey FROM complaints ORDER BY id DESC").all();
  return json(results || []);
});
router.post("/api/complaints", async (req, env) => {
  const body = await readBody(req);
  const created = body.created || (/* @__PURE__ */ new Date()).toISOString();
  const r = await env.DB.prepare("INSERT INTO complaints(name, identity, content, created, fileName, fileKey) VALUES(?, ?, ?, ?, ?, ?)").bind(
    body.name || "",
    body.identity || "",
    body.content || "",
    created,
...
router.get("/api/suggestions", async (req, env) => {
  if (!isAdmin(req, env)) return unauthorized();
  const { results } = await env.DB.prepare("SELECT id, name, identity, content, created FROM suggestions ORDER BY id DESC").all();
  return json(results || []);
});
router.post("/api/suggestions", async (req, env) => {
  const body = await readBody(req);
  const created = body.created || (/* @__PURE__ */ new Date()).toISOString();
  const r = await env.DB.prepare("INSERT INTO suggestions(name, identity, content, created) VALUES(?, ?, ?, ?)").bind(body.name || "", body.identity || "", body.content || "", created).run();
  return json({ ok: true, id: r.meta?.last_row_id || null });
});
